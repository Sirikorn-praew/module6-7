...
def Move(m,list):
    ...
    print(list)
    for i in list:    
        byteL = toByte(i[0],i[1],i[2],i[3],255)
        if m == "PID":
            data = [255,244,byteL[0],byteL[1],byteL[2],byteL[3],byteL[4],byteL[5],byteL[6],byteL[7],byteL[8]]    #"FF,F4" + x + y + z
        if m == "Traj":
            data = [255,245,byteL[0],byteL[1],byteL[2],byteL[3],byteL[4],byteL[5],byteL[6],byteL[7],byteL[8]]    #"FF,F5" + x + y + z
            ...
            if len(a) != 0:
                ...
                if m == "Traj":
                    if spic == 245:
                        break    
        #print("Move to x="+ x +" , y="+ y + " , z="+ z + " , a="+ a + " , g="+ g)
        msg = "Move to x="+ str(i[0]) +" , y="+ str(i[1]) + " , z="+ str(i[2]) + " , a="+ str(i[3]) + " , g="+ str(255)
        mySW.SetMsg(msg) 
...
class MainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.lunalink = LUNALink()
        self.movePID = MovePID()
        self.startLUNALink()

    def startMovePID(self):
        self.movePID.setupUI(self)
        self.movePID.CPSBTN.clicked.connect(self.startLUNALink)
        self.show()

    def startLUNALink(self):
        self.lunalink.setupUI(self)
        self.lunalink.ToolsBTN.clicked.connect(self.startMovePID)
        self.show()
